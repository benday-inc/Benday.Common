...
<#
.SYNOPSIS
    Builds the Benday.Common solution and copies the NuGet packages to the local NuGet folder.
...
dotnet build Benday.Common.slnx --configuration $Configuration
...
Get-ChildItem -Path $localNuGet -Filter "Benday.Common*.nupkg" |
    Sort-Object Name |
